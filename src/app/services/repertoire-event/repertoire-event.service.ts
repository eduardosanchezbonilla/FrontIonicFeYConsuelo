...
export class RepertoireEventService {
  ...
  async updateRepertoireEventOrderNumbers(repertoireEvent:RepertoireEvent){
    const token = await this.storage.getItem('token');
    return Http.put(
      {
        url:environment.host + '/repertoire-march/' + repertoireEvent.marchId + '/event/'+ repertoireEvent.eventType + '/' + repertoireEvent.eventId + '/order/' + repertoireEvent.order+ '/numbers/' + repertoireEvent.nubmers,
        params:{},
        data:{},
        headers: {
          'Content-Type': 'application/json',
      ...
      if(response.status==200 || response.status==201){        
        ...
        return Promise.reject({
          status: response.status,
          message: response.data?.message || 'Error al actualizar el orden de la marcha asociada al evento'
        });
      }      
    })
    .catch((error) => {    
